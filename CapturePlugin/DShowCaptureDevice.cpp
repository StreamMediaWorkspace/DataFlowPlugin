...
void DShowCaptureDevice::ListCapDevices(const IID& deviceIID, std::map<std::string, std::string>& deviceList)
{
    ...
    while (true)
    {
        ...
        deviceList[Utils::WString2String(wszDeviceID)] = Utils::WString2String(var.bstrVal);
...
void DShowCaptureDevice::ListVideoCapDeviceWH(const std::string& vDeviceID,
    std::vector<int>& widthList, std::vector<int>& heightList)
{
    ...
    if (SUCCEEDED(hr)) {
        ULONG ulFetched;
        CComPtr<IMoniker> moniker;
        hr = ::MkParseDisplayName(bind_ctx, Utils::String2WString(vDeviceID).c_str(), &ulFetched, &moniker);
